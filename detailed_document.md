...
Team of three. V. Abhishek Prakash wrote the code, models and data pipeline; Akhil Prasad handled manual testing and submission logistics, and Hrishikesh did manual testing.
...
is the project's answer to alert fatigue: a fault
...
`docs/claims_evidence.md` exactly. **[SOURCE-SCALE]** means the size of the
originating production CMS, evidenced by aggregate-count CSVs under
`data/reference/` but *not* reproducible from the delivered slice.
**[VALIDATED]** means reproducible from the committed repo right now, with a
...
predictive maintenance (flag 17).
...
- **Real-event detection: 100% per category**, with err1024 99/99, GroundFailure
...
- **The 13-second finding, confirmed on real data:** across 190 real err1051
...
Every limitation below is documented in a committed source.
